...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        #         der = wrap_tuple(der)
        #         for d in der:
        #             out = VariableWithDeriv(variable=inp, deriv=d)
        #             outputs.append(out)
        # return outputs

        d_inps = cls.backward(ctx, d_output)
        d_inps = wrap_tuple(d_inps)
        return [
            VariableWithDeriv(inp, d_in)
            for inp, d_in in zip(inputs, d_inps)
            if not is_constant(inp)
        ]
...
def backpropagate(final_variable_with_deriv):
    ...
    while queue:
        q = queue.pop(0)
        if is_leaf(q.variable):
            q.variable._add_deriv(q.deriv)
        else:
            v = q.variable
            h = v.history
            for out in h.backprop_step(q.deriv):
                if not is_constant(out.variable):
                    queue.append(out)
